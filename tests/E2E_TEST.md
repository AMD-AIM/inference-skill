...
```bash
cd ~/inference-skill
python3 tests/e2e_optimize_test.py --output-dir ~/inferencex_<config-key>_<timestamp>/
```
...
```bash
cd ~/inference-skill
python3 tests/e2e_optimize_test.py
```
...
python3 tests/e2e_optimize_test.py --target vllm
python3 tests/e2e_optimize_test.py --target sglang

# Validate only (no pipeline run)
python3 tests/e2e_optimize_test.py --output-dir <path>
```
