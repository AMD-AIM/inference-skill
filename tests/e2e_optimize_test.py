...
def check_benchmark(output_dir, config):
    ...
    for rf in result_files:
        with open(rf) as f:
            data = json.load(f)
        tps = data.get("total_token_throughput", data.get("output_throughput", 0))
...
def check_kernel_optimize(output_dir, config):
    ...
    if os.path.isfile(geak_path):
        ...
        if isinstance(results, list):
            ...
            entries = results.get("results", results.get("entries", [results]))
        else:
            entries = []
...
def check_integration(output_dir, config):
    ...
    if os.path.isfile(geak_path):
        ...
        geak_entries = geak_data if isinstance(geak_data, list) else geak_data.get("results", geak_data.get("entries", []))
...
def generate_report(output_dir, config, checks, issues):
    ...
    with open(md_path, "w") as f:
        ...
        if issues:
            f.write("\n## Detected Issues\n\n")
            # Deduplicate by (source, pattern) for readability
            seen = set()
            unique_issues = []
            for issue in issues:
...
def validate_output_dir(output_dir):
    ...
    if issues:
        ...
        for i in error_issues:
            key = (i.source, i.pattern)
            if key not in seen:
                seen.add(key)
                unique_errors.append(i)
