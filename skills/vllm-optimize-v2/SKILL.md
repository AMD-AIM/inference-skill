...
- Print `=== Phase N/6: <name> — STARTING ===` before each phase.
- Print `=== Phase N/6: <name> — DONE ===` with a 1-line key metric after each phase.
- Print `=== Phase N/6: <name> — FAILED: <reason> ===` immediately on any error, then stop.
- All long-running commands (benchmark, profiling, tuning) must use `tee` to write to a per-phase log file AND stream to stdout simultaneously. Never run silently.
- Heartbeat every 30s during waits (server startup, benchmark runs): print elapsed time + last log line.
- Per-phase log files: `{{OUTPUT_DIR}}/logs/phase_N_<name>.log` — create this dir in workspace bootstrap.
- The `{{PROGRESS_FILE}}` must be updated at the START and END of each phase so a crashed run is diagnosable.
