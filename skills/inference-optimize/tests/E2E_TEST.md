# E2E Test: inference-optimize

Automated end-to-end testing for the inference-optimize skill pipeline.
...
```bash
# Kill any leftover inference containers
docker ps -q --filter label=inference-pipeline | xargs -r docker stop
docker ps -aq --filter label=inference-pipeline | xargs -r docker rm
...
```bash
claude "Use inference-optimize for gptoss-fp4-mi355x-vllm with optimize workflow, TP=8, 1k1k, conc=4"
```

**SGLang (671B MoE, ~45-90 min):**
```bash
claude "Use inference-optimize for dsr1-fp4-mi355x-sglang with optimize workflow, TP=8, 1k1k, conc=4"
```
...
| # | Phase | What happens |
|---|-------|-------------|
| 0 | env-setup | Checks Docker, GPUs, benchmark repo, HF cache |
...
```
~/inference_<config-key>_<YYYYMMDD>_<HHMMSS>/
```

Example: `~/inference_gptoss-fp4-mi355x-vllm_20260330_140000/`

### Step 4: Validate the output

```bash
python3 ~/.claude/skills/inference-optimize/tests/e2e_optimize_test.py \
  --output-dir ~/inference_<config-key>_<timestamp>/
```
...
```bash
cat ~/inference_<config-key>_<timestamp>/test_report.md
```
...
python3 ~/.claude/skills/inference-optimize/tests/e2e_optimize_test.py --target vllm
python3 ~/.claude/skills/inference-optimize/tests/e2e_optimize_test.py --target sglang

# Validate only (no pipeline run)
python3 ~/.claude/skills/inference-optimize/tests/e2e_optimize_test.py --output-dir <path>
```
...
```bash
docker logs inference-profile-<config-key> 2>&1 | tail -50
```
