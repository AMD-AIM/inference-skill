...
class TestNoDormantFields:
    ...
    def test_structured_predicates_have_valid_ops(self):
        ...
        reg = _load_registry()
        valid_ops = {"eq", "ne", "lt", "le", "gt", "ge", "in", "not_in", "exists", "not_exists"}
        for key, phase in reg["phases"].items():
...
# ---------------------------------------------------------------------------
# V2 monitor configuration invariants
# ---------------------------------------------------------------------------

class TestV2MonitorConfig:
    """V2 monitor configuration invariants."""

    def test_v2_monitor_default_false(self):
        reg = _load_registry()
        assert reg.get("v2_monitor") is False, "v2_monitor must default to false"

    def test_human_loop_default_false(self):
        reg = _load_registry()
        assert reg.get("human_loop") is False, "human_loop must default to false"

    def test_redirect_abort_not_in_verdict_values(self):
        """REDIRECT and ABORT are response actions, never verdicts."""
        reg = _load_registry()
        for key, phase in reg["phases"].items():
            rules = phase.get("quality", {}).get("detection_rules_structured", [])
    ...
    def test_v2_rules_have_categories(self):
        """All V2 detection rules must have a category field."""
        reg = _load_registry()
        valid_cats = {"infrastructure", "logic", "data_quality", "performance_regression",
        ...
        for key, phase in reg["phases"].items():
            ...
            for rule in rules_v2:
                assert "category" in rule, (
                    f"Phase {key}: V2 rule missing category field"
                )
