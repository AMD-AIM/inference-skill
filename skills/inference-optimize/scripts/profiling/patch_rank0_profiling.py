...
def patch_vllm():
    ...
    paths = glob.glob(
        "/usr/local/lib/python3.*/dist-packages/vllm/v1/worker/gpu_worker.py"
    )
    if not paths:
        print("WARNING: vLLM gpu_worker.py not found, skipping rank-0 patch")
        return
    path = paths[0]
    with open(path) as f:
        content = f.read()
    ...
    if old in content:
        content = content.replace(
            old,
            "if True  # force-mount profiler routes (patched by inference-optimize)",
            1,
        )
        with open(path, "w") as f:
            f.write(content)
