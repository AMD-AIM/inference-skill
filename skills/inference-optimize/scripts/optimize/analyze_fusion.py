#!/usr/bin/env python3
"""
Operator Fusion Analysis for inference optimization pipeline.
...
NOTE: analyze_fusion_inferencex.py is deprecated; use this file instead.
...
def load_tracelens_kernel_summary(tracelens_dir):
    ...
    with open(path) as f:
        for row in csv.DictReader(f):
            kernels.append({
                "name": row.get("Kernel name", row.get("name", "")),
                "parent_op": row.get("Parent cpu_op", ""),
                "parent_category": row.get("Parent op category", ""),
                "calls": int(float(row.get("Kernel duration (us)_count", row.get("Calls", 0)) or 0)),
                "total_us": float(row.get("Kernel duration (us)_sum", row.get("Total time (us)", 0)) or 0),
            })
    return kernels
...
def build_bottleneck_kernels(kernels, tracelens_kernels, threshold_pct):
    ...
    for k in kernels:
        ...
        bottlenecks.append({
            ...
            "optimizable": kernel_type not in SKIP_KERNEL_TYPES,
        })
...
def main():
    parser = argparse.ArgumentParser(description="Analyze fusion opportunities from profiling data")
