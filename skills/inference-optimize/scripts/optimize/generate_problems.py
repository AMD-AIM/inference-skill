#!/usr/bin/env python3
"""
Problem file generator for the inference optimization pipeline.
...
Generates problem files from profiling outputs (gap_analysis.json + TraceLens CSVs).

NOTE: generate_problems_inferencex.py is deprecated; use this file instead.
...
from effort_budget import compute_effort_budget


def _majority_bound(items, key="bound"):
    """Return the most common bound value from a list of dicts, defaulting to 'unknown'."""
    bounds = [item.get(key, "unknown") for item in items]
    if not bounds:
        return "unknown"
    return max(set(bounds), key=bounds.count)
...
def generate_manifest(output_dir, fusions_path, bottlenecks_path, framework,
    ...
    if gemm_groups:
        for grp in gemm_groups:
            ...
            manifest["optimizations"].append({
                ...
                "bound": _majority_bound(grp.get("shapes", [])),
                "enabled": True,
            })
    ...
    # Compute effort budgets from roofline data
    for opt in manifest["optimizations"]:
        budget = compute_effort_budget(
            roofline_efficiency=opt.get("roofline_efficiency"),
            bound=opt.get("bound"),
        )
        opt["max_optimization_attempts"] = budget.max_optimization_attempts
        opt["max_infra_retries"] = budget.max_infra_retries
...
def main():
    parser = argparse.ArgumentParser(description="Generate problem files from profiling data")
