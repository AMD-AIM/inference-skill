...
The emitted `optimization_summary.json` aggregates high-level fields consumed by dashboards: `config_key`, `framework`, `pipeline_status` (`completed`, `completed with warnings`, `completed with blockers`, or `pipeline incomplete`), `all_phases_completed` (boolean — true when `pipeline_status` is `completed` or `completed with warnings`; this replaces the former `phases_completed` boolean to avoid collision with the array-based `progress.json.phases_completed`), optional `gpu_arch` / `vllm_version` (when `env-info` parses), throughput + `speedup` + `e2e_speedup` + `validated` + `performance_gate` + `ttft_regression_pct` from `optimization_comparison.json`, integration-manifest fields under the library-rebuild contract (`integration_schema_version`, `libraries_rebuilt_count`, `libraries_rebuilt`, `dispatch_verified`, `e2e_ran`), dispatch-verification counters (`expected_symbol_total_count`, `vendor_symbol_leaked_count`, `redirect_required_count`, `redirect_honored_count`), per-kernel stats (`kernels_attempted`, `kernels_improved`, `unverified_per_kernel_count`, `bucket_b_winners`, embedded `kernel_results`), manifest sizing (`total_optimization_targets`, `bucket_a_count`, `bucket_b_count`, `bucket_c_count`), and forks/pin status from `forks/manifest.json` (`forks_required_count`, `forks_pinned_count`, `ck_branch_merged_status`). Extend the Python emitter only when new machine fields are required—keep the Markdown report as the human narrative.
