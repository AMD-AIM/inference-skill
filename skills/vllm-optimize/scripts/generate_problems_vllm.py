#!/usr/bin/env python3
"""
Problem file generator for the vLLM optimization pipeline.
...
Usage:
    python generate_problems_vllm.py \
...
Part of the vllm-optimize skill. Can be used standalone.
"""
import argparse
import csv
import json
...
def generate_fusion_problems(fusions_path, model_shapes, output_dir):
    ...
    if not os.path.isfile(fusions_path):
        print("No fusion_opportunities.json -- run analyze_fusion_vllm.py first")
        return 0
...
def main():
    parser = argparse.ArgumentParser(description="Generate problem files from vLLM profiling data")
